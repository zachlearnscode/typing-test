<template>
  ...
  >
    ...
    <text-input
      ...
      :prompt="prompt"
      :disabled="disabled || status == 'Time Expired'"
      ref="textInput"
    ...
    <v-dialog v-model="dialog" max-width="300">
      <v-card>
        <v-card-title class="text-no-wrap">Time's Up!</v-card-title>
        <v-card-text class="text-h5">
          You typed {{ wordsPerMinute() }} words per minute, with a total of
          {{ numErrors() }}.</v-card-text
        >
        <v-card-actions>
          <v-btn
            block
            @click="[(prompt = ''), (textInput = ''), $emit('dialog-ok')]"
            >OK</v-btn
          >
        </v-card-actions>
      </v-card>
  ...
  methods: {
    ...
    selectPrompt() {
      let randomIndex = Math.floor(Math.random() * 9),
        prompt = this.prompts[randomIndex];

      return (this.prompt = prompt);
    },
    numErrors() {
      let numErrors = document.querySelectorAll(".error").length;

      return numErrors != 1 ? `${numErrors} errors` : `${numErrors} error`;
    },
    wordsPerMinute() {
      let numErrors = document.querySelectorAll(".error").length;

      let wpm = (this.textInput.length / 5 - numErrors).toFixed(1);
